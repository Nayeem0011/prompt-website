...
function PromptModal({ prompt, onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-[#0f0f18] border border-white/[0.1] rounded-2xl w-full max-w-lg p-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input value={form.title} onChange={(e) => update("title", e.target.value)} placeholder="Prompt title..." className="w-full bg-white/[0.05] border border-white/[0.08] rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/20 outline-none focus:border-fuchsia-500/45 transition-all" />
          </div>
          <div>
            <label className="text-xs font-medium text-white/45 mb-1.5 block">Category</label>
            <select value={form.category} onChange={(e) => update("category", e.target.value)} className="w-full bg-white/[0.05] border border-white/[0.08] rounded-xl px-4 py-2.5 text-sm text-white outline-none focus:border-fuchsia-500/45 transition-all appearance-none">
          ...
          <div>
            ...
            <textarea value={form.description} onChange={(e) => update("description", e.target.value)} rows={3} placeholder="Describe what this prompt does..." className="w-full bg-white/[0.05] border border-white/[0.08] rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/20 outline-none focus:border-fuchsia-500/45 transition-all resize-none" />
          </div>
          <div>
            <label className="text-xs font-medium text-white/45 mb-1.5 block">Tags (comma separated)</label>
            <input value={Array.isArray(form.tags) ? form.tags.join(", ") : form.tags} onChange={(e) => update("tags", e.target.value)} placeholder="Midjourney, Portrait, Cinematic" className="w-full bg-white/[0.05] border border-white/[0.08] rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/20 outline-none focus:border-fuchsia-500/45 transition-all" />
          </div>
          <div>
            <label className="text-xs font-medium text-white/45 mb-1.5 block">Status</label>
            <div className="flex gap-2">
              {["active", "paused", "draft"].map((s) => (
                <button key={s} onClick={() => update("status", s)} className={`px-4 py-2 rounded-xl text-xs font-medium capitalize transition-all border ${form.status === s ? "border-fuchsia-500/50 bg-fuchsia-500/10 text-fuchsia-300" : "border-white/[0.08] text-white/35 hover:text-white/60"}`}>{s}</button>
              ))}
            </div>
          </div>
        </div>
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-white/[0.08] text-sm font-medium text-white/50 hover:text-white/80 transition-all">Cancel</button>
          {/* TODO: call /api/prompts POST or PUT */}
          <button onClick={() => { onSave(form); onClose(); }} className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-600 text-sm font-semibold text-white hover:opacity-90 transition-all">
...
function StatusBadge({ status }) {
  const map = {
    ...
    paused: "text-white/30 bg-white/[0.05]",
...
export default function MyPromptsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-[#0f0f18] border border-white/[0.1] rounded-2xl p-6 max-w-sm w-full text-center">
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteId(null)} className="flex-1 py-2.5 rounded-xl border border-white/[0.08] text-sm text-white/50 hover:text-white/80 transition-all">Cancel</button>
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setModal("add")} className="flex items-center gap-2 bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:opacity-90 rounded-xl px-4 py-2.5 text-sm font-semibold text-white transition-all">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex gap-1 bg-white/[0.03] border border-white/[0.07] rounded-xl p-1">
          {Object.entries(counts).map(([key, count]) => (
            <button key={key} onClick={() => setFilter(key)} className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-all flex items-center gap-1.5 ${filter === key ? "bg-white/[0.09] text-white" : "text-white/35 hover:text-white/60"}`}>
              {key}
              <span className={`text-[10px] px-1.5 py-0.5 rounded-md ${filter === key ? "bg-white/10 text-white/70" : "bg-white/[0.05] text-white/25"}`}>{count}</span>
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 relative min-w-48">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search prompts..." className="w-full bg-white/[0.04] border border-white/[0.07] rounded-xl pl-9 pr-4 py-2 text-sm text-white placeholder-white/20 outline-none focus:border-fuchsia-500/35 transition-all" />
        </div>
        ...
        <div className="flex gap-1 bg-white/[0.03] border border-white/[0.07] rounded-xl p-1">
          {[["grid", <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><rect x="1" y="1" width="4.5" height="4.5" rx="1" stroke="currentColor" strokeWidth="1.2"/><rect x="7.5" y="1" width="4.5" height="4.5" rx="1" stroke="currentColor" strokeWidth="1.2"/><rect x="1" y="7.5" width="4.5" height="4.5" rx="1" stroke="currentColor" strokeWidth="1.2"/><rect x="7.5" y="7.5" width="4.5" height="4.5" rx="1" stroke="currentColor" strokeWidth="1.2"/></svg>], ["list", <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M1.5 3.5h10M1.5 6.5h10M1.5 9.5h10" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/></svg>]].map(([v, icon]) => (
            <button key={v} onClick={() => setView(v)} className={`p-1.5 rounded-lg transition-all ${view === v ? "bg-white/[0.09] text-white" : "text-white/30 hover:text-white/60"}`}>{icon}</button>
          ))}
        </div>
      </div>
      ...
      {view === "grid" && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((prompt) => (
            <div key={prompt.id} className="group bg-white/[0.03] border border-white/[0.07] hover:border-white/[0.12] rounded-2xl overflow-hidden transition-all duration-200">
              {/* Card visual */}
              <div className={`h-32 bg-gradient-to-br ${accentColors[prompt.category] || "from-white/5 to-white/10"} relative flex items-center justify-center`}>
              ...
              <div className="p-4">
                ...
                <div className="flex items-center gap-3 mt-3 pt-3 border-t border-white/[0.05] text-xs text-white/30">
          ...
          {/* Empty add card */}
          <button onClick={() => setModal("add")} className="border border-dashed border-white/[0.1] hover:border-fuchsia-500/30 rounded-2xl h-52 flex flex-col items-center justify-center gap-3 text-white/20 hover:text-fuchsia-400 transition-all group">
            <div className="w-10 h-10 rounded-xl bg-white/[0.03] group-hover:bg-fuchsia-500/10 border border-white/[0.07] group-hover:border-fuchsia-500/30 flex items-center justify-center transition-all">
            ...
            <tbody>
              {filtered.map((p) => (
                <tr key={p.id} className="border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-5 py-4">
                    <div className="flex gap-1 flex-wrap">{p.tags.map((t) => <span key={t} className="text-[10px] bg-white/[0.05] text-white/35 px-2 py-0.5 rounded-md">{t}</span>)}</div>
                  </td>
