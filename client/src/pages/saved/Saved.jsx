...
const mockSaved = [
  { id: 1, title: "Cinematic Portrait Lighting", category: "Portraits", author: "arif_prompts", copies: 2841, rating: 4.9, tags: ["Midjourney", "Portrait"], savedAt: "2025-05-20", description: "A powerful prompt for generating cinematic, dramatic portrait lighting with deep shadows and warm key light.", promptText: "A cinematic portrait with dramatic Rembrandt lighting, deep shadows on one side of the face, warm golden key light, shot on 85mm lens, bokeh background, ultra-realistic skin texture, professional photography..." },
  { id: 2, title: "Retro Synthwave Landscape", category: "Posters & Visuals", author: "neon_creator", copies: 1540, rating: 4.8, tags: ["Stable Diffusion", "Synthwave"], savedAt: "2025-05-18", description: "Create stunning retrowave landscapes with neon grids, purple sunsets, and chrome typography.", promptText: "A retro synthwave landscape at dusk, neon purple and pink horizon, chrome grid road stretching into infinity, palm tree silhouettes, 80s aesthetic..." },
  { id: 3, title: "Watercolor Brand Identity", category: "Brand & Logo", author: "brandmaster", copies: 890, rating: 4.7, tags: ["DALL-E", "Watercolor"], savedAt: "2025-05-15", description: "Generate beautiful watercolor-style brand identity elements for artisanal and boutique brands.", promptText: "A delicate watercolor brand identity, soft pastel tones, hand-painted texture, flowing organic shapes, botanical elements, premium artisanal aesthetic..." },
  { id: 4, title: "Cyberpunk Street Scene", category: "Illustration & 3D", author: "pixel_wizard", copies: 3200, rating: 5.0, tags: ["Midjourney", "Cyberpunk"], savedAt: "2025-05-10", description: "Ultra-detailed cyberpunk street environments with neon signs, rain reflections, and futuristic architecture.", promptText: "A rainy cyberpunk alley at night, neon signs reflecting on wet pavement, holographic advertisements, detailed futuristic architecture, volumetric lighting, ultra-detailed 8K..." },
  { id: 5, title: "Minimalist Product Photography", category: "Ads & Product", author: "studio_ai", copies: 670, rating: 4.6, tags: ["DALL-E", "Product"], savedAt: "2025-05-05", description: "Clean, professional product photography prompts for e-commerce and marketing.", promptText: "A minimalist product photography setup, pure white background, soft studio lighting from the left, shallow depth of field, professional commercial photography style..." },
];
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-[#0c0c16] border-l border-white/8 h-full overflow-y-auto flex flex-col">
        {/* Drawer header */}
        <div className={`h-40 bg-linear-to-br ${accentBg[prompt.category] || "from-white/5 to-white/10"} flex items-end p-5 flex-shrink-0 relative`}>
        ...
        <div className="flex items-center gap-4 px-5 py-4 border-b border-white/[0.06]">
          ...
          <div className="flex gap-1 flex-wrap">
            {prompt.tags.map((t) => <span key={t} className="text-[10px] bg-white/[0.06] text-white/40 px-2 py-0.5 rounded-md">{t}</span>)}
          </div>
        </div>
        ...
        <div className="px-5 py-4 flex-1">
          ...
          <div className="bg-white/[0.04] border border-white/[0.07] rounded-xl p-4">
            <p className="text-sm text-white/70 leading-relaxed font-mono text-xs">{prompt.promptText}</p>
          </div>
        </div>
 
        {/* Actions */}
        <div className="px-5 py-4 border-t border-white/[0.06] flex gap-3 flex-shrink-0">
          <button onClick={handleCopy} className={`flex-1 flex items-center justify-center gap-2 rounded-xl py-3 text-sm font-semibold transition-all ${copied ? "bg-emerald-500/20 border border-emerald-500/30 text-emerald-400" : "bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:opacity-90 text-white"}`}>
            {copied ? (
          ...
          <button onClick={() => { onUnsave(prompt.id); onClose(); }} className="px-4 py-3 rounded-xl border border-white/[0.08] text-white/40 hover:text-red-400 hover:border-red-500/25 hover:bg-red-500/5 transition-all">
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-64">
      <div className="flex flex-col items-center gap-3">
        ...
        <p className="text-sm text-white/30">Loading saved prompts...</p>
      </div>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-48">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search saved prompts..." className="w-full bg-white/[0.04] border border-white/[0.07] rounded-xl pl-9 pr-4 py-2 text-sm text-white placeholder-white/20 outline-none focus:border-fuchsia-500/35 transition-all" />
        </div>
        <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-white/[0.04] border border-white/[0.07] rounded-xl px-3 py-2 text-sm text-white/60 outline-none focus:border-fuchsia-500/35 transition-all appearance-none pr-8 cursor-pointer">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
        {categories.map((cat) => (
          <button key={cat} onClick={() => setActiveCategory(cat)} className={`flex-shrink-0 px-4 py-2 rounded-xl text-xs font-medium transition-all border ${activeCategory === cat ? "border-fuchsia-500/40 bg-fuchsia-500/10 text-fuchsia-300" : "border-white/[0.07] bg-white/[0.03] text-white/35 hover:text-white/60 hover:bg-white/[0.05]"}`}>
            {cat}
      ...
      {filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="w-16 h-16 bg-white/[0.03] border border-white/[0.07] rounded-2xl flex items-center justify-center mb-4">
      ...
      {filtered.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((prompt) => (
            <div key={prompt.id} onClick={() => setSelected(prompt)} className="group bg-white/[0.03] border border-white/[0.07] hover:border-white/[0.13] rounded-2xl overflow-hidden cursor-pointer transition-all duration-200 hover:-translate-y-0.5">
              {/* Card top */}
              <div className={`h-28 bg-gradient-to-br ${accentBg[prompt.category] || "from-white/5 to-white/10"} relative flex items-center justify-center`}>
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-white/[0.05] text-xs text-white/28">
